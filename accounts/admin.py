...
class CompanyAdmin(PlatformOnlyAdminMixin, admin.ModelAdmin):
    ...
    change_form_template = "admin/accounts/company/change_form.html"
    ...
    def get_urls(self):
        ...
        custom_urls = [
            path(
                "<path:object_id>/disable-all-modules/",
                self.admin_site.admin_view(self.disable_all_modules_view),
                name="%s_%s_disable_all_modules" % info,
            ),
        ]
    ...
    def changeform_view(self, request, object_id=None, form_url="", extra_context=None):
        ...
        if object_id and request.user.can_manage_companies():
            extra_context["disable_all_modules_url"] = reverse(
                f"{self.admin_site.name}:{self.opts.app_label}_{self.opts.model_name}_disable_all_modules",
                args=[object_id],
            )
        return super().changeform_view(request, object_id, form_url, extra_context)

    def disable_all_modules_view(self, request, object_id):
        if not request.user.can_manage_companies():
            raise PermissionDenied

        company = self.get_object(request, object_id)
        if company is None:
            return redirect(f"{self.admin_site.name}:accounts_company_changelist")

        if request.method == "POST" and request.POST.get("confirm") == "yes":
            previous_values = {
                "can_use_tagging": company.can_use_tagging,
                "can_use_inventory": company.can_use_inventory,
                "can_use_queueing": company.can_use_queueing,
            }
            company.can_use_tagging = False
            company.can_use_inventory = False
            company.can_use_queueing = False
            company.save(update_fields=["can_use_tagging", "can_use_inventory", "can_use_queueing"])
            self.log_change(request, company, "Disabled all modules for tenant.")
            create_audit_log(
                actor=request.user,
                action="TENANT_MODULES_DISABLED",
                target_model="accounts.Company",
                target_id=company.pk,
                description=f"Disabled all modules for tenant {company.name}.",
                changes={
                    "before": previous_values,
                    "after": {
                        "can_use_tagging": False,
            ...
            self.message_user(
                request,
                f"All modules have been disabled for {company.name}.",
                messages.SUCCESS,
            )
            return redirect(
                reverse(f"{self.admin_site.name}:{self.opts.app_label}_{self.opts.model_name}_change", args=[company.pk])
            )
        ...
        context = {
            **self.admin_site.each_context(request),
            ...
            "original": company,
            "object_id": object_id,
            "title": "Confirm disable all modules",
            "index_url": reverse(f"{self.admin_site.name}:index"),
            "app_list_url": reverse(f"{self.admin_site.name}:app_list", kwargs={"app_label": self.opts.app_label}),
            "changelist_url": reverse(f"{self.admin_site.name}:{self.opts.app_label}_{self.opts.model_name}_changelist"),
            "change_url": reverse(f"{self.admin_site.name}:{self.opts.app_label}_{self.opts.model_name}_change", args=[object_id]),
            "disable_all_modules_url": reverse(
                f"{self.admin_site.name}:{self.opts.app_label}_{self.opts.model_name}_disable_all_modules",
                args=[object_id],
            ),
        }
        return TemplateResponse(request, "admin/accounts/company/disable_all_modules_confirmation.html", context)
...
class TenantScopedCompanyAdmin(CompanyAdmin):
    def has_module_permission(self, request):
        return tenant_admin_site.has_permission(request)
    ...
    def has_change_permission(self, request, obj=None):
        if request.user.can_manage_companies():
            return True
        return request.user.role == User.Role.SUPER_ADMIN and request.user.company_id is not None
    ...
    def get_readonly_fields(self, request, obj=None):
        readonly_fields = list(super().get_readonly_fields(request, obj))
        if not request.user.can_manage_companies():
            readonly_fields.extend(["name", "code", "is_active", "can_use_tagging", "can_use_inventory", "can_use_queueing"])
        return tuple(dict.fromkeys(readonly_fields))


tenant_admin_site.register(Company, TenantScopedCompanyAdmin)
